...
class WD_Shopbycolor {
	...
	public function wd_pc_color_color_column( $columns, $column, $id ) {
		...
		if ( $column == 'color' ) {
			...
			$datas = get_metadata( 'woocommerce_term', $id, "wd_pc_color_config", true );
			if( strlen($datas) > 0 ){
	...
	public function wd_pc_color_fields_save( $term_id, $tt_id, $taxonomy ){
		...
		$result = update_metadata( 'woocommerce_term',$term_id,"wd_pc_color_config",$_term_config_str );

	}

	public function wd_pc_color_fields_remove( $term_id, $tt_id, $taxonomy ){
		delete_metadata( 'woocommerce_term',$term_id,"wd_pc_color_config" );
	}	
...
class WD_Widget_PC_Color_Nav extends WP_Widget {
	 ...
	 * @return void
	 */
	function WD_Widget_PC_Color_Nav() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'wpdance_color_nav', $this->wpdance_widget_name, $widget_ops );
	}
		...
		if ( count( $terms ) > 0 ) {
			...
			if(1) {
				...
				foreach ( $terms as $term ) {
					...
					echo ( $count > 0 || $option_is_set ) ? '<a title="'. $term->name .'" href="' . esc_url( apply_filters( 'wpdance_color_nav_link', $link ) ) . '">' : '<span>';
					...
					if( absint($datas['wd_pc_color_image']) > 0  ){
						...
						echo "<span style='background-color:{$datas['wd_pc_color_color']}'>{$term->name}</span>";
					}					
