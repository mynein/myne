	...
	function accordions($atts,$content){
		...
		if( $tabs_match && is_array($match) && count($match) > 0 ){
			...
			for( $_count = 0 ; $_count < count($match[0]) ; $_count++ ){
				...
				$_accordions_title .= "\n\t\t\t<a class=\"accordion-toggle\" href=\"#{$_content_id}\" data-parent=\"#{$iden_accordion_box}\" data-toggle=\"collapse\">{$match[1][$_count]}</a>";
				$_accordions_title .= "\n\t\t</div>";
		...
		$result = "<div class='accordion ".$color."' id='{$iden_accordion_box}'>".$_accordions_html."</div>";
