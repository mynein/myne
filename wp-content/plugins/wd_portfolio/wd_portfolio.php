...
class WD_Portfolio {
		...
		<div class="wd-portfolio-meta-section">
			<div class="form-wrap">
				<div class="form-field">
					<label for="wd_portfolio"><?php _e('Image/Video URL', 'wd_portfolio_context')?></label>
					<input type="text" id="wd_portfolio" name="wd_portfolio" value="<?php echo htmlspecialchars(self::wd_portfolio_get_meta('wd-portfolio')); ?>" style="width:70%;" />
					<a id="wd_portfolio_media_lib" href="javascript:void(0);" class="button" rel="wd_portfolio">URL from Media Library</a>
					<p><?php _e('Enter URL for the full-size image or video (youtube, vimeo, swf, quicktime) you want to display in the lightbox gallery. You can also choose Image URL from your Media gallery', 'wd_portfolio_context')?></p>
				</div>            
				<div class="form-field">
					<label for="wd_portfolio_url"><?php _e('Portfolio URL', 'wd_portfolio_context')?></label>
					<input type="text" name="wd_portfolio_url" value="<?php echo htmlspecialchars(self::wd_portfolio_get_meta('wd-portfolio-url')); ?>" />
					<p><?php _e('Enter URL to the live version of the project.', 'wd_portfolio_context')?></p>
				</div>   
	...
	public function wd_portfolio_register_taxonomies() {
		...
		if (count(get_terms('wd-portfolio-category', 'hide_empty=0')) == 0) {
			register_taxonomy('category', 'portfolio', array('hierarchical' => true, 'label' => 'Portfolio Category'));
	...
	public function wd_portfolio_create_section() {
		add_meta_box('wd-portfolio-section-options', __('Options', 'wd_portfolio_context'), array($this,'wd_portfolio_section_options') , 'portfolio', 'normal', 'high');
	}
	...
	public function rename_second_menu_name($safe_text, $text) {
		if (__('Portfolio Items', 'WD_portfolio_context') !== $text) {
			return $safe_text;
		}
		...
		return __('WD Portfolio', 'wd_portfolio_context');
	}
	...
	public function wd_portfolio_show($atts = array()) {
		...
		show_wd_portfolio(  $columns,$show_filter,$style,$show_categories,$show_title,$show_desc,$count, $show_pagination, $category );	
	}


	protected function init_trigger(){
		add_image_size('portfolio_image',600,440,true); 
	}
