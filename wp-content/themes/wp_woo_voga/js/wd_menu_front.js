...
function menu_change_state( case_size ){
	...
	if( jQuery('#header').length == 0 )
		return;
	if( jQuery('#header').hasClass('wd-boxed') ){
		var _container = jQuery('.header-top');
		var _submenu_width = _container.width() + 6;
		var _container_offset_left = _container.offset().left + 12;
	}
	else{
		var _container = jQuery('.header-top .header-top-content');
		var _submenu_width = _container.width() + 6;
		var _container_offset_left = _container.offset().left + 12;
	}
	var _container_width = _container.width() + 6;
	var _container_outer_width = _container.width() + 6;
		...
		if( case_size < 768 ){ 
			...
			setTimeout(function(){
				...
				jQuery('.nav > .main-menu > ul.menu').children('.menu-item-level0.wd-mega-menu.columns-5,.menu-item-level0.wd-mega-menu.columns-4,.menu-item-level0.wd-mega-menu.columns-3,.menu-item-level0.wd-mega-menu.columns-2').each(function(index,value){
					...
					jQuery(value).children('ul.sub-menu').css({'margin-left':-sub_menu_width/2+'px','left':'50%'});
					
					var container_left = _container.offset().left;
					var container_right = container_left + _container_width;
					var item_left = jQuery(value).offset().left;
					var item_width = jQuery(value).outerWidth();
					
					var overflow_left = (sub_menu_width/2 > (item_left - container_left + item_width/2));
	...
	/* Mobile menu action */
	jQuery('.wd_mobile_menu_wrapper .menu-icon').removeClass('active');
	jQuery('.wd_mobile_menu_content').hide();
