    ...
    <li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
        <dl class="menu-item-bar">
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item','wpdance'); ?>" href="<?php
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            ...
            <p class="description description-thin">
			 ...
			 <p class="field-sup-label description description-thin wd-custom-menu wd-add-on-lv0">
                <label for="edit-menu-item-sup-label-<?php echo esc_attr($item_id); ?>">
                    <?php _e( 'Sup Label','wpdance' ); ?><br />
                    <input type="text" id="edit-menu-item-sup-label-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-sup-label" name="menu-item-sup-label[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->sup_label ); ?>" />
                </label>
            </p>
			
			<div class="input-append color colorpicker-menu description wd-custom-menu wd-add-on-lv0" data-color="<?php echo esc_attr( $item->sup_label_background_color ); ?>" data-color-format="hex">
				<label><?php _e( 'Sup Label Background Color','wpdance' ); ?></label>
				<input id="edit-menu-item-sup-label-background-color-<?php echo esc_attr($item_id); ?>" name="menu-item-sup-label-background-color[<?php echo esc_attr($item_id); ?>]" id="theme-color" type="text" class="span2 widefat code edit-menu-item-wide-custom-color" value="<?php echo esc_attr( $item->sup_label_background_color ); ?>" >
				<span class="add-on"><i style="color: <?php echo esc_attr( $item->sup_label_background_color ); ?>"></i></span>
			</div>					
            ...
            <p class="field-wide-style description description-thin wd-custom-menu wd-add-on-lv0">
                <label for="edit-menu-item-wide-style-<?php echo esc_attr($item_id); ?>">
					<?php $wide_style = esc_attr( $item->wide_style );?>
                    <?php _e( 'Mega Menu','wpdance' ); ?><br />
                    <!--<input type="text" id="edit-menu-item-wide-style-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-wide-style" name="menu-item-wide-style[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->wide_style ); ?>" />-->
					<select id="edit-menu-item-wide-style-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-wide-style" name="menu-item-wide-style[<?php echo esc_attr($item_id); ?>]">
			...
			<p class="field-wide-column description description-thin wd-custom-menu wd-add-on-lv0 mega-active">
                <label for="edit-menu-item-wide-column-<?php echo esc_attr($item_id); ?>">
					<?php $wide_column = esc_attr( $item->wide_column );?>
                    <?php _e( 'Columns','wpdance' ); ?><br />
					<select id="edit-menu-item-wide-column-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-wide-column" name="menu-item-wide-column[<?php echo esc_attr($item_id); ?>]">
			...
			<p class="field-sub-column description description-thin wd-custom-menu wd-add-on-lv1 parrent-active depend-field-wide-column">
                <label for="edit-menu-item-sub-column-<?php echo esc_attr($item_id); ?>">
					<?php $sub_column = esc_attr( $item->sub_column );?>
                    <?php _e( 'Sub Columns','wpdance' ); ?><br />
					<select id="edit-menu-item-sub-column-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-sub-column" name="menu-item-sub-column[<?php echo esc_attr($item_id); ?>]">
			...
			<?php if( $numSideBar > 0):?>
					<p class="field-wide-widget description description-thin wd-custom-menu wd-add-on-lv0 wd-add-on-lv1 mega-active parrent-active">
						<label for="edit-menu-item-sidebars-<?php echo esc_attr($item_id); ?>">
            ...
            <p class="field-thumbnail description description-thin wd-custom-menu wd-add-on-lv1 wd-add-on-lv2 wd-add-on-lv0 parrent-active">
                <label>
					...
					<a id="<?php echo esc_attr($item_id); ?>" class="custom_upload_image_button_mega" href="javascript:void(0)" style="display:<?php echo ($have_thumb ? 'none' : 'inline'); ?>;">Select Image</a>
           			<span class="edit-menu-item-thumbnail-wrapper">
						<?php if($have_thumb):?>
							<img src="<?php echo esc_url($menu_thumbnail); ?>" width="32" height="32" id="thumbnail-menu-item-<?php echo esc_attr($item_id); ?>" title="menu-item-<?php echo esc_attr($item_id); ?>-thumbnail" alt="menu-item-<?php echo esc_attr($item_id); ?>-thumbnail">
						<?php endif?>
					</span>
					<a id="<?php echo esc_attr($item_id); ?>" class="custom_clear_image_button_mega" href="javascript:void(0)" style="display:<?php echo ($have_thumb ? 'inline' : 'none'); ?>;">Remove Image</a>
					<input type="hidden" id="edit-menu-item-thumbnail-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-thumbnail" name="menu-item-thumbnail[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->thumbnail ); ?>" />
					<input type="hidden" id="edit-menu-item-thumbnail-id-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-thumbnail-id" name="menu-item-thumbnail-id[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->thumbnail_id ); ?>" />
				 </label>	
               
            </p>	

            <p class="field-wide-column description description-thin wd-custom-menu wd-add-on-lv0 mega-active" >
                <label for="edit-menu-item-wide-align-right-<?php echo esc_attr($item_id); ?>">
					<?php 
						$align_right = (int) $item->align_right;
					?>
                    <?php _e( 'Align Right','wpdance' ); ?><br />
					<select id="edit-menu-item-align-right-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-align-right" name="menu-item-align-right[<?php echo esc_attr($item_id); ?>]">
					   <option value="0" <?php if($align_right == 0) echo "selected";?> >No</option>
					   <option value="1" <?php if($align_right == 1) echo "selected";?> >Yes</option>
					</select> 
					
                </label>
            </p>
			
			<p class="field-static-html description description-thin wd-custom-menu">
                <label for="edit-menu-item-static-html-<?php echo esc_attr($item_id); ?>">
                    <?php _e( 'Static HTML','wpdance' ); ?><br />
					<?php wp_editor( stripslashes(htmlspecialchars_decode( base64_decode($item->static_html) )), 'menu-item-static-html['.$item_id.']' ); ?>
                </label>
            </p> 
            ...
            <div class="menu-item-actions description-wide submitbox">
                ...
                ); ?>"><?php _e('Remove','wpdance'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php _e('Cancel','wpdance'); ?></a>
            </div>
