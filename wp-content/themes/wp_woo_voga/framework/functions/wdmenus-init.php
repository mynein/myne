...
function wd_mega_menu_script(){	
	...
	wp_enqueue_script( 'wd_some_handle',THEME_JS.'/wd_menu_front.js' );
	$translation_array = array( 'wd_vertical_menu' => __( 'shop by departments', 'wpdance' ) );
	wp_localize_script( 'wd_some_handle', 'wd_object_name', $translation_array );
}
...
// add_action( 'after_setup_theme', 'wd_nav_setup', 600);
// function wd_nav_setup(){
	// add_theme_support('post-thumbnails', array('post','nav_menu_item','page'));
...
function em_register_sidebars(){
	if(function_exists('register_sidebars')){
		global $wd_data;
		$numSidebars = (isset($wd_data['wd_menu_num_widget']) && (int)$wd_data['wd_menu_num_widget']> 0) ? (int)$wd_data['wd_menu_num_widget'] : 1;
		if($numSidebars > 0 ){
			if($numSidebars == 1){
				register_sidebar(array(
					'name'          => 'Wd Mega Menu Widget Area 1',
					'id'            => 'wd-mega-sidebar',
				...
				register_sidebars( $numSidebars, array(
					'name'          => 'Wd Mega Menu Widget Area %d',
					'id'            => "wd-mega-sidebar",
					'before_title'  => '<h2 class="widgettitle">',
					'after_title'   => '</h2>' 
				));
...
function custom_nav_update($menu_id, $menu_item_db_id, $args ) {
    ...
    if ( isset($_REQUEST['menu-item-align-right']) && is_array($_REQUEST['menu-item-align-right']) ) {
        update_post_meta( $menu_item_db_id, WD_MEGA_MENU.'_menu_item_align_right', $_REQUEST['menu-item-align-right'][$menu_item_db_id] );
    }
	
	if ( isset($_REQUEST['menu-item-static-html']) && is_array($_REQUEST['menu-item-static-html']) ) {
        update_post_meta( $menu_item_db_id, WD_MEGA_MENU.'_menu_item_static_html', base64_encode( htmlspecialchars( $_REQUEST['menu-item-static-html'][$menu_item_db_id] ) ) );
    }
	
	if ( isset($_REQUEST['menu-item-sup-label']) && is_array($_REQUEST['menu-item-sup-label']) ) {
        update_post_meta( $menu_item_db_id, WD_MEGA_MENU.'_menu_item_sup_label', $_REQUEST['menu-item-sup-label'][$menu_item_db_id] );
    }
	if ( isset($_REQUEST['menu-item-sup-label-background-color']) && is_array($_REQUEST['menu-item-sup-label-background-color']) ) {
        update_post_meta( $menu_item_db_id, WD_MEGA_MENU.'_menu_item_sup_label_background_color', $_REQUEST['menu-item-sup-label-background-color'][$menu_item_db_id] );
    }
...
function custom_nav_item($menu_item) {
	...
	$menu_item->align_right = get_post_meta( $menu_item->ID, WD_MEGA_MENU.'_menu_item_align_right', true );
	$menu_item->static_html = get_post_meta( $menu_item->ID, WD_MEGA_MENU.'_menu_item_static_html', true );
	$menu_item->sup_label = get_post_meta( $menu_item->ID, WD_MEGA_MENU.'_menu_item_sup_label', true );
	$menu_item->sup_label_background_color = get_post_meta( $menu_item->ID, WD_MEGA_MENU.'_menu_item_sup_label_background_color', true );
    return $menu_item;
}
