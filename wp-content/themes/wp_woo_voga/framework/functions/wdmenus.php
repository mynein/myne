...
class WD_Walker_Nav_Menu extends Walker_Nav_Menu{
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		/*if( $item->menu_order == 1 ){
			$output .= $indent . '<li class="menu-item-0 menu-item-level-0 menu-dropdown" id="wd-menu-item-dropdown" style="display:none;"></li>'; 
		}*/
		$class_names = $value = '';
		...
		if($depth < 3){
			...
			if( (int) $item->sub_count > 0 || strlen($static_html) > 0 || ( $this->parent_mega_enable == true && strlen($side_bar) > 0 ) ){
	...
	function end_el( &$output, $item, $depth = 0, $args = array() ) {
		...
		**/
		...
		if( $this->parent_mega_enable == true ){
			...
			$static_html = get_post_meta( $item->ID, WD_MEGA_MENU.'_menu_item_static_html', true );
			$static_html = stripslashes(htmlspecialchars_decode( base64_decode($static_html) ));
			$parent_static_html = get_post_meta( $parent_id, WD_MEGA_MENU.'_menu_item_static_html', true );
			$parent_static_html = stripslashes(htmlspecialchars_decode( base64_decode($parent_static_html) ));
			...
			if( $depth < 3 && $depth > 0  && ( strlen($parent_side_bar) > 0 || strlen($parent_static_html) > 0 ) ){
				...
				if( strlen($parent_static_html) > 0 ){
					if ( $this->lv0_subcount == $this->lv1_order && $depth == 1){
						$class = 'wpmega-nonlink';
						$class.= ' wpmega-static-html';	
						$this->parent_static_html_str = '';
						
						$this->parent_static_html_str .= '<div class="'.$class.'">';
						$this->parent_static_html_str .= do_shortcode( $parent_static_html );
						$this->parent_static_html_str .= '<div class="clear"></div>';	
						$this->parent_static_html_str .= '</div>';

						$output .= "{$indent}</li>\n";
						$output .= "{$indent}<li class='mega-new-line'></li>\n";
						$output .= "{$indent}<li class=\"static-html-menu static-html-" . ($depth). "\">\n" . $this->parent_static_html_str . "{$indent}\n"; 
						$output .= "{$indent}\t";
			...
			if( $depth == 0 && $this->lv0_subcount == 0 ){
				if( $this->parent_sidebar == true || $this->parent_static_html == true ){
					...
					if( $this->parent_static_html == true ){
						if( $this->parent_sidebar == true ){
							$output .= "{$indent}<li class='mega-new-line'></li>";
						}
						$class = 'wpmega-nonlink';
						$class.= ' wpmega-static-html';	
						$static_html_str = '';
						$static_html_str .= '<div class="'.$class.'">';
						$static_html_str .= do_shortcode( $static_html );
						$static_html_str .= '<div class="clear"></div>';	
						$static_html_str .= '</div>';
						$output .= "{$indent}<li class=\"static-html-menu static-html-" . ($depth). "\">\n" . $static_html_str . "{$indent}</li>\n"; 
					}
