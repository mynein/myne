...
if(!class_exists('WP_Widget_EmAds')){
	...
	class WP_Widget_EmAds extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct('emads', __('WD - Ads','wpdance'), $widget_ops);
		}
		...
		function widget( $args, $instance ) {
			...
			<div class="em-ads-widget"><?php //echo $instance['filter'] ? wpautop($text) : $text; ?>
				<a href="<?php echo esc_url($url) ?>"><img src="<?php echo esc_url($img) ?>"  alt="<?php echo esc_attr($img_title) ?>" title="<?php echo esc_attr($img_title) ?>" <?php echo $subHtml ?> /></a>
			</div>
			<?php
			echo $after_widget;
		}
