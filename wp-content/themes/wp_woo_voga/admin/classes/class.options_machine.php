...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($smof_data) || !empty($wd_data)){
			...
			switch ( $value['type'] ) {
				...
				case 'button':
					$output .= '<a class="button">' . $value['std'] . '</a>';
				break;
				...
				// Color update for ajax styling options
				case "color-update":
					$default_color = '';
					if ( isset($value['std']) ) {
						$default_color = ' data-default-color="' .$value['std'] . '" ';
					}
					$output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $value['std']. '"' . $default_color .' />';
				...
				case "color-rgba":
					...
					//$output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color-rgba"  type="text" value="' . $smof_data[$value['id']] . '"' . $default_color .' />';
					$output .= '<div class="input-append color of-color-rgba" data-color="'.$smof_data[$value['id']].'" data-color-format="hex">
					<input name="' . $value['id'] . '" id="' . $value['id'] . '" type="text" class="span2 colorpicker_control_rgba" value="'.$smof_data[$value['id']].'" >
					<span class="add-on"><i style="background-color: '.$smof_data[$value['id']].'>"></i>Select Color</span>
				</div>';
				break;
				//typography option	
				case 'typography':
				...
				case 'sorter':
				    ...
				    $sortlists = isset($wd_data[$value['id']]) && !empty($wd_data[$value['id']]) ? $wd_data[$value['id']] : $value['std'];
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','wpdance').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'select_google_font':
					$position = '';
					if(isset($value['position']) && $value['position'] != '') { $position = $value['position']; }
					$output .= '<div class="select_wrapper '.$position.'">';
