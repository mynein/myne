...
jQuery(document).ready(function($){
	...
	//add color is rgba
	//$('.of-color-rgba').colorpicker({ 'format':"rgba" });
	...
	$('.of-radio-img-img').live('click',function(){
		...
		var color_file = $(this).siblings('.of-radio-img-radio').val();
		if($(this).closest("#of-option-stylingoptions").length > 0){
			var loading_popup = $('#of-popup-loading');
			loading_popup.fadeIn(0);
			...
			$.post(ajaxurl, data, function(response) {			
				...
				if(response==-1){ //failed	
					...
					window.setTimeout(function(){
						...
						$('#of-option-stylingoptions').replaceWith('<div class="group" id="of-option-stylingoptions" style="display: block;"><h2>Styling Options</h2>' + JSON.parse(response));
						$('.of-color').wpColorPicker();
		...
		img_selected.parent().parent().find('.of-radio-img-img').removeClass('of-radio-img-selected');
		img_selected.addClass('of-radio-img-selected');	
	});
	$('.of-radio-img-label').hide();
	$('.of-radio-img-img').show();
	$('.of-radio-img-radio').hide();
	...
	/**
	  * Switchs
	  * Dependencies 	 : jquery
	  * Feature added by : WD team - http://wpdance.com/
	  * Date 			 : 03.17.2013
	  */
	jQuery(".cb-one").click(function(){
		var parent = $(this).parents('.switchs-options');
		jQuery('.cb-two',parent).removeClass('selected');
		jQuery(this).addClass('selected');
		jQuery('.main_checkbox',parent).attr('checked', true);
		...
		var $fold='.f_'+obj.data('id');
		jQuery($fold).slideUp('normal', "swing");
		if(jQuery($fold).find('.select_wrapper.left').length > 0) {
			jQuery($fold).has('.select_wrapper.left').slideDown('normal', "swing");
		}
	});
	jQuery(".cb-two").click(function(){
		var parent = $(this).parents('.switchs-options');
		jQuery('.cb-one',parent).removeClass('selected');
		jQuery(this).addClass('selected');
		jQuery('.main_checkbox',parent).attr('checked', false);
		...
		var $fold='.f_'+obj.data('id');
		jQuery($fold).slideUp('normal', "swing");
		if(jQuery($fold).find('.select_wrapper.right').length > 0) {
			jQuery($fold).has('.select_wrapper.right').slideDown('normal', "swing");
		}
	});
	...
	function stickyInfo() {
        ...
        if (!jQuery('#info_bar').isOnScreen() && !jQuery('#wd-footer-theme-option-sticky').isOnScreen()) {
            jQuery('#wd-footer-theme-option').css({
                position: 'fixed'
                ,bottom: '0'
                ,width: stickyWidth
				,background: '#eee'
            ...
            jQuery('#wd-footer-theme-option').addClass('sticky-footer-fixed');
        } else {
            jQuery('#wd-footer-theme-option').css({
                background: '#f3f3f3'
                ,position: 'inherit'
            ...
            jQuery('#wd-footer-theme-option').removeClass('sticky-footer-fixed');
        }
    }

    if (jQuery('#wd-footer-theme-option').length !== 0) {
		...
		jQuery('#of_form #main #of-nav ul li a').bind('click',function(){
			stickyInfo();
		});
		stickyInfo();
    }
	...
	jQuery('#section-wd_import_button .button').bind('click', function(e){
		e.preventDefault();
		var ok = confirm('WARNING: Importing maybe duplicate your content. Importing data is recommended on fresh installs only once');
		if( !ok ){
			return;
		}
		
		var mask = '<div class="import-mask"><div class="loading"><i class="fa fa-spinner fa-spin"></i></div></div>';
		jQuery('body').append(mask);
		
		var data = {
			action 						: 'import_demo_content'
			,import_pages_posts_menu 	: jQuery('#wd_import_pages_posts').val()
			,import_revsliders 			: jQuery('#wd_import_revsliders').val()
			,import_widgets 			: jQuery('#wd_import_widgets').val()
		};
		jQuery.ajax({
			type : 'POST'
			,url : ajaxurl
			...
			,error: function(xhr, err){
				jQuery('.import-mask').remove();
				alert('Import failed');
			}
			,success : function(response){
				if( response != '0' ){ /* Success */
					jQuery('.import-mask').remove();
					alert('Importing is complete. Your site will be reloaded');
					location.reload();
				}
				else{
					jQuery('.import-mask').remove();
